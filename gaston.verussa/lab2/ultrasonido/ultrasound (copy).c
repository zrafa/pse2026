...
volatile unsigned char * pin_trigger;

volatile unsigned char trigger_pin_number;

/* dirección de PORT Trigger */
volatile unsigned char * port_echo;
...
volatile unsigned char echo_pin_number;
...
void ultrasound_init(char trigger_num, char echo_num)
{
	if(trigger_num >= 0 && trigger_num <= 5){
		// Del 0 al 8 se considera puerto B, el pin 6 y 7 no se usa
		port_trigger = (unsigned char *) PUERTO_B;
		ddr_trigger = (unsigned char *) DDR_B;
		pin_trigger = (unsigned char *) PIN_B;
		trigger_pin_number = trigger_num;
	} else if (trigger_num >= 8 && trigger_num <= 13){
		// Del 0 al 8 se considera puerto C, el pin 6 y 7 no se usa
		port_trigger = (unsigned char *) PUERTO_C;
		ddr_trigger = (unsigned char *) DDR_C;
		pin_trigger = (unsigned char *) PIN_C;
		trigger_pin_number = trigger_num - 8;
		...
		pin_trigger = (unsigned char *) PIN_D;
		trigger_pin_number = trigger_num - 16;
	}
	
	*ddr_trigger |= (1 << trigger_pin_number);
	*ddr_echo &= ~(1 << echo_pin_number);
}
...
int ultrasound_get_distance(void)
{
	...
	*puerto_trigger &= ~(1 << trigger_pin_number);
	...
	*puerto_trigger &= ~(1 << trigger_pin_number);
	...
	while (!(*pin_echo & (1 << echo_pin_number)) && timeout > 0) {
		timeout--;
	}
	...
	while (*pin_echo & (1 << echo_pin_number)) {
		duracion++;
		_delay_us(1);
