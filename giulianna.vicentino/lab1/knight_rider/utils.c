/* Enciende y apaga un led conectado al puerto B bit 5 de un atmega328p
 * El puerto B de un atmega328p tiene los bits 0-5 mapeados a los 
 * pines 8-13 de arduino 
 */

// puertos de E/S

volatile unsigned char * puerto_b = (unsigned char *) 0x25;// direccion de PORTB (registro de datos)
volatile unsigned char * ddr_b = (unsigned char *) 0x24;// direccion de DDR B (registro de control) 
volatile unsigned char * pin_b = (unsigned char *) 0x23;// direccion PIN B (registro de datos de entrada)

void esperar() {
	volatile unsigned long i;
	/* delay de 1 segundo */
	for (i=0; i<50000; i++);
}

/* led_init: configura el puerto b bit 5 como salida */
void led_init() {
	*(puerto_b) = 0;
	*(ddr_b) = 0b00011111;
}

/* led_on: enciende el led conectado al puerto b bit 5 */
void cambiar(char estado) {
	*(puerto_b) = estado;
}
