...
void thread_pse(controllerClass & control) {
    // 1. Abrir el puerto como en el código de la derecha
    ...
    while (1) {
        ...
        if (n > 0) {
            if (c == '\n') {
                buf[idx] = '\0'; // Terminamos el texto colocando el carácter nulo

		if (buf[0] == 's') {
                    control.btnPress();
                } 
                else if (idx > 0) {
                    int pos_arduino = std::atoi(buf);
                    
                  
                   float valor_norm = (float)pos_arduino / 1023.0f;
                   float valor_corregido = std::pow(valor_norm, 0.3f);//linealizacion
                   float px = (valor_corregido * 2.9f) - 1.45f;

		    
                    control.movePaddle(px);
                }
                
                idx = 0; // Reiniciamos el índice del buffer para el próximo dato
            } 
    ...
    close(fd); // Buenas prácticas: cerrar el puerto al salir
}
