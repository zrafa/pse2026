...
void thread_pse(controllerClass & control) {
  ...
  while (1) {
    int raw = read_int();
    char shoot = read_char();

    // mapear ADC (0–1024) a rango del juego (-1.66 a 1.66)
    float move = ((float)raw / 1024.0f) * (2 * 1.66f) - 1.66f;

    control.movePaddle(move);

    // actualizar lgica interna
    control.get();
