...
#include <termios.h>  /* Para configurar el puerto serie */
...
int serial_fd = -1;   /* Descriptor del puerto */

int init_serial(){
   /* Abrimos el serial */
   serial_fd = open("/dev/ttyUSB0", O_RDONLY | O_NOCTTY | O_NONBLOCK);
   ...
   if (serial_fd != -1) {
      ...
      /* Configurar velocidad a 9600 baudios */
      cfsetispeed(&options, B9600);
      cfsetospeed(&options, B9600);

      /* Modo "Raw": sin eco, sin procesar señales, lectura byte a byte */
...
/*Defino la función para recibir caracteres por el dispositivo serial*/
char getcharacter() {
   char c;
   read(serial_fd, &c, 1);
   return c;
