...
#include <fcntl.h>
#include <unistd.h>
#include <termios.h> // <-- LIBRERÍA CLAVE PARA EVITAR EL CONGELAMIENTO
...
void thread_pse(controllerClass & control) {
    ...
    if (serial_fd < 0) {
        std::cout << "Error abriendo /dev/ttyUSB0. ¿Permisos de sudo?" << std::endl;
        return;
    }
    ...
    // --- VARIABLES DEL FILTRO ANTIRRUIDO ---
    float valor_filtrado = 0.5f;   // Empieza en el medio
    const float suavizado = 0.15f; // Nivel de "amortiguador" (0.01 es muy lento, 1.0 es sin filtro)

    while (1) {
        if (read(serial_fd, &byte_leido, 1) > 0) {
            
            // --- POTENCIÓMETRO ---
            if (byte_leido == 'K') {
                unsigned char valor_knob;
                while (read(serial_fd, &valor_knob, 1) <= 0); 
                
                float normalizado = valor_knob / 255.0f;
                
                // Aplicamos el filtro: mezcla un 15% de la lectura nueva con un 85% de la lectura vieja
                valor_filtrado = (suavizado * normalizado) + ((1.0f - suavizado) * valor_filtrado);
                
                float linealizado = pow(valor_filtrado, 0.3f); 
                float move = (linealizado * 3.2f) - 1.6f;
                
                control.movePaddle(move);
                control.get(); 
            }
            // --- BOTÓN ---
            else if (byte_leido == 'B') {
                unsigned char valor_btn;
                while (read(serial_fd, &valor_btn, 1) <= 0); 
                
                if (valor_btn == 1 && last_btn == 0) {
                    control.btnPress(); 
                }
                last_btn = valor_btn;
            }
